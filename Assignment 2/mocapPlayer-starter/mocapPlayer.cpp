...
void saveScreenToFile_callback(Fl_Button *button, void *)
{
  if (button == screenShot_button)
  {
    ...
    saveScreenshot(640 * 2, 480 * 2, saveScreenToFileOnceFilename);
...
void saveScreenshot(int windowWidth, int windowHeight, char * filename)
{
  ...
  for (int i=windowHeight-1; i>=0; i--) 
  {
    ...
    //use absolute value to store image
    char fullName[FILENAME_MAX] = "/Users/haha/Downloads/csci520-assignment2-startercode/Video/Linear_Quaternion/";
    strcat(fullName, filename);
    //printf("%s\n",filename);
  if (ppm_write(fullName, in))
...
void idle(void*)
{
  ...
  if(playButton == ON) 
  {
    if (saveScreenToFile == SAVE_CONTINUOUS)
    {
      ...
      saveScreenshot(640 * 2, 480 * 2, saveScreenToFileContinuousFilename);
