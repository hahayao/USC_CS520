    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  // Also useful are the Mat3d and RigidTransform4d classes defined in the Vega folder.
    ...
    for(int i = 0; i < jointUpdateOrder.size(); i++)
    {
        ...
        //R_O * R_L
        //convert to 4 * 4
        int currentID = jointUpdateOrder[i];
        euler2Rotation(jointOrientationEulerAngles[currentID], R_O, rotateOrders[currentID]);
        euler2Rotation(eulerAngles[currentID], R_L, rotateOrders[currentID]);
        localTransforms[currentID] = RigidTransform4d(Mat3d(R_O) * Mat3d(R_L), translations[currentID]);
    ...
    for(int i = 0; i < jointUpdateOrder.size(); i++)
    {
        ...
        //root joint
        if(jointParents[currentID] == -1)
        {
            globalTransforms[currentID] = localTransforms[currentID];
        }
        //xParent = localTransform * xLocal
        else
        {
            globalTransforms[currentID] = globalTransforms[jointParents[currentID]] * localTransforms[currentID];
        }
    }
}
