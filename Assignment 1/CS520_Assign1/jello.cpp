...
int selectedSurface = 6;

static int countScreenNum = 0;
...
void myinit()
{
  ...
  glEnable(GL_LINE_SMOOTH);
    ...
    CreateBoundaryEquation(&jello);
    CreateStructuralSpring();
    CreateBendSpring();
    CreateDiagonalSpring();
...
void doIdle()
{
  ...
  int i;
    char absoluteS[100] = "/Users/haha/Documents/Prac_C++/CS520_Assign1/CS520_Assign1/pic/";
...
strcat(absoluteS, s);
    ...
    //animation is 
    if(countScreenNum % jello.n == 0)
    {
        if (saveScreenToFile==1)
        {
          saveScreenshot(windowWidth, windowHeight, absoluteS);
          //saveScreenToFile=0; // save only once, change this if you want continuos image generation (i.e. animation)
          sprite++;
            cout<<countScreenNum<<endl;
        }
    }
    countScreenNum++;
...
void TimerFunc( int t )
{
    ...
    doIdle();
    glutPostRedisplay();
    glutTimerFunc(1000 * jello.dt, TimerFunc, 0);
}
