  ...
  switch (button)
  {
    case GLUT_LEFT_BUTTON:
      g_iLeftMouseButton = (state==GLUT_DOWN);
          ...
          if(state == GLUT_UP)
          {
              cout<<"You are adding a force to this jello."<<endl;
              int current_x = g_vMousePos[0];
              int current_y = g_vMousePos[1];
              cout<<"The force is from ("<<x<<", "<<y<<") to ("<<current_x<<", "<<current_y<<")"<<endl;
              UserForce[0] = current_x - x;
              UserForce[1] = current_y - y;
          }
      break;
...
*/
  ...
  for (i= 0; i <= 7 ; i++)
  {
    ...
    //cout<<jello->p[0][0][0].x<<jello->p[0][0][0].y<<jello->p[0][0][0].z<<endl;
    //cout<<jello->p[7][7][7].x<<jello->p[7][7][7].y<<jello->p[7][7][7].z<<endl;
